...
final class ElevenLabsEngine {
    ...
    private func requestChunk(
        ...
        let task = session.dataTask(with: request) { [weak self] data, response, error in
            ...
            if let httpResponse = response as? HTTPURLResponse, !(200..<300).contains(httpResponse.statusCode) {
                self.finishAfterChunkError(
                    ElevenLabsSpeechError.requestFailed(
                        "ElevenLabs request failed (HTTP \(httpResponse.statusCode))."
                    ),
                    didEnqueueAudio: didEnqueueAudio,
                    completion: completion
                )
    ...
    private func finishAfterChunkError(
        ...
        elevenLabsLogger.error("ElevenLabs chunk failed afterAudio=\(didEnqueueAudio, privacy: .public) error=\(error.localizedDescription, privacy: .private)")
        DispatchQueue.main.async { [weak self] in
            guard let self else { return }
            self.activeTask = nil
            if didEnqueueAudio {
