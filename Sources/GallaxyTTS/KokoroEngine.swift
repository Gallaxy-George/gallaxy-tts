...
final class KokoroEngine {
    ...
    private static let modelRevision = "a71e4d38b236d968966a2002c4c895dbd12b1c3c"
    ...
    func warmUp() {
        do {
            ...
            kokoroLogger.error("Kokoro warmup failed error=\(error.localizedDescription, privacy: .private)")
        }
    }
    ...
    func keepWarm() {
        do {
            ...
            kokoroLogger.error("Kokoro keepalive failed error=\(error.localizedDescription, privacy: .private)")
            resetWorker()
        }
    }
    ...
    func prepareAfterWake() {
        do {
            ...
            kokoroLogger.error("Kokoro wake refresh failed error=\(error.localizedDescription, privacy: .private)")
        }
    }
    ...
    func speak(
        ...
        do {
            ...
            // This path is only for installations without the persistent worker.
            // Retrying a failed worker as a fresh process reloads the model and can leave
            // the next request waiting for minutes.
            stopOneShotProcess()
            ...
            process.arguments = invocation.leadingArguments + [
                ...
                "--model", Self.modelID,
                "--model-revision", Self.modelRevision,
                "--voice", Self.voiceID,
